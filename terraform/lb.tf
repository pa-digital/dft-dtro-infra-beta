...
resource "google_compute_url_map" "default" {
  ...
  host_rule {
    hosts        = [var.consume_service_domain]
    path_matcher = "consume"
  }

  path_matcher {
    name            = "consume"
    default_service = module.loadbalancer.backend_services["consume"].id
  }
  ...
  test {
    host    = var.consume_service_domain
    path    = "/health"
    service = module.loadbalancer.backend_services["consume"].id
  }
}
...
module "loadbalancer" {
  ...
  https_redirect                  = true
  managed_ssl_certificate_domains = [var.publish_service_domain, var.consume_service_domain]
  ...
  backends = {
    ...
    consume = {
      description = "${var.consume_service_image} service"

      groups = [
        {
          group = google_compute_region_network_endpoint_group.consume_service_serverless_neg.id
        }
      ]
